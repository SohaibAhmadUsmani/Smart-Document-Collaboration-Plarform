...
/**
 * WorkspaceCardSkeleton Component
 * English: Skeleton placeholder while workspace records are loading.
 * Roman Urdu: Workspaces load hone ke doran skeleton placeholder card dikhata hai.
 */
 ...
 *
 * English:
 * Displays all workspaces belonging to the authenticated user with role badges,
 * modal triggers to create new workspaces, safe fallback back navigation, and responsive layouts.
 *
 * Roman Urdu:
 * Authenticated user ke tamam workspaces dikhata hai unke roles ke sath, naya workspace banane ka modal,
 * smart back navigation, aur responsive layout provide karta hai.
 *
 * @returns {JSX.Element}
 */
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-6 sm:py-8">
        ...
        {error && (
          <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            {error}{' '}
            <button className="font-medium underline cursor-pointer" onClick={refresh}>
              Try again
            </button>
          </div>
        )}
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Create your first workspace to start organizing folders and collaborating with your team."
            action={
              <Button variant="primary" onClick={() => setCreateOpen(true)}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {workspaces.map((workspace) => (
              <Link
                ...
                className="group rounded-xl border border-border bg-surface p-5 shadow-card transition-shadow hover:shadow-popover"
              >
